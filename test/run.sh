...
if [[ $RUN_LINT -eq 1 ]]; then
  ...
  for f in CameraView.qml FullscreenVideo.qml GcodeWatcher.qml PrinterConnection.qml PrinterIcon.qml Service.qml; do
...
run_qml_test() {
  ...
  if [[ -f "test/qml/$name.needs-shell" ]]; then
    ...
    cp CameraView.qml PrinterIcon.qml Panel.qml FullscreenVideo.qml "$stage/"
  fi
